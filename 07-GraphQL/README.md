...
cat alive-domains.txt | httpx -silent -threads 100 -path /graphql,/api/graphql,/gql,/query -x POST -H "Content-Type: application/json" -body '{"query":"{__typename}"}' -mc 200,400,403 | tee graphql-raw.txt | grep -Ei "typename|errors|graphql" | anew graphql-endpoints.txt
